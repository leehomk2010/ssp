...
public class GlobalSettingController extends BaseController{
	...
	/** 列表 */
	@RequestMapping
	public String index(ModelMap model,GlobalSettingQuery query,HttpServletRequest request) {
		setDefaultSortColumns(query,DEFAULT_SORT_COLUMNS);
		Page page = globalSettingManager.findPage(query);
	...
	public String show(ModelMap model,@PathVariable Integer id) throws Exception {
		GlobalSetting globalSetting = (GlobalSetting)globalSettingManager.getById(id);
		model.addAttribute("globalSetting",globalSetting);
		return "/globalsetting/show";
	}

	/** 进入新增 */
	@RequestMapping(value="/new")
	public String _new(ModelMap model,GlobalSetting globalSetting) throws Exception {
		model.addAttribute("globalSetting",globalSetting);
		return "/globalsetting/new";
	}
	
	/** 保存新增,@Valid标注spirng在绑定对象时自动为我们验证对象属性并存放errors在BindingResult  */
	@RequestMapping(method=RequestMethod.POST)
	public String create(ModelMap model,GlobalSetting globalSetting,BindingResult errors) throws Exception {
		try {
			globalSettingManager.save(globalSetting);
	...
	public String edit(ModelMap model,@PathVariable Integer id) throws Exception {
		GlobalSetting globalSetting = (GlobalSetting)globalSettingManager.getById(id);
		model.addAttribute("globalSetting",globalSetting);
		return "/globalsetting/edit";
	}
	
	/** 保存更新,@Valid标注spirng在绑定对象时自动为我们验证对象属性并存放errors在BindingResult  */
	@RequestMapping(value="/{id}",method=RequestMethod.PUT)
	public String update(ModelMap model,@PathVariable Integer id,GlobalSetting globalSetting,BindingResult errors) throws Exception {
		try {
			globalSettingManager.update(globalSetting);
		}catch(ConstraintViolationException e) {
			convert(e, errors);
			return  "/globalsetting/edit";
		}catch(ValidationException e) {
	...
	public String delete(ModelMap model,@PathVariable Integer id) {
		globalSettingManager.removeById(id);
		return LIST_ACTION;
	}
	...
	public String batchDelete(ModelMap model,@RequestParam("items") Integer[] items) {
		for(int i = 0; i < items.length; i++) {
			globalSettingManager.removeById(items[i]);
		}
		return LIST_ACTION;
	}
