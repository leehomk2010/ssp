...
public class ReportController extends BaseController{
	...
	/** 列表 */
	@RequestMapping
	public String index(ModelMap model,ReportQuery query,HttpServletRequest request) {
		setDefaultSortColumns(query,DEFAULT_SORT_COLUMNS);
		Page page = reportManager.findPage(query);
	...
	public String _new(ModelMap model,Report report) throws Exception {
		model.addAttribute("report",report);
		return "/report/new";
	}
	...
	public String create(ModelMap model,Report report,BindingResult errors) throws Exception {
		try {
	...
	public String batchDelete(ModelMap model,@RequestParam("items") Long[] items) {
		for(int i = 0; i < items.length; i++) {
			reportManager.removeById(items[i]);
		}
		return LIST_ACTION;
	}
